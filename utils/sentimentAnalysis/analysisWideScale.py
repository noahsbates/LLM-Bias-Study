...
def nlptownSentimentWideIndividual(sentiment_pipe, sentence):
    pres_sentiment = sentiment_pipe(sentence,top_k=None)
    totalRating = 0
...
def nlptownSentimentWide(path):
    ...
    for president_name in tqdm.tqdm(president_df['name'], desc="Getting sentiment..."):
        ...
        for i in tqdm.tqdm(president_poems, desc=f"Analyzing {president_name}..."):
            totalRating = nlptownSentimentWideIndividual(sentiment_pipe, i)
...
def cardiffnlpSentimentWideIndividual(sentiment_pipe, sentence):
    pres_sentiment = sentiment_pipe(sentence, top_k=None)
    totalRating = 0
    for label in pres_sentiment:
...
def cardiffnlpSentimentWide(path):
    ...
    for president_name in tqdm.tqdm(president_df['name'], desc="Getting sentiment..."):
        ...
        for i in tqdm.tqdm(president_poems, desc=f"Analyzing {president_name}..."):
            totalRating = cardiffnlpSentimentWideIndividual(sentiment_pipe, i)
